...
sub run {
    my $self  = shift;
    my @data  = @_;
    my @rooms = map { $self->to_room($_) } @data;

    my $id_sum = 0;
    for my $room (@rooms) {
        if ($self->is_real_room($room)) {
            print $room->{original}, " is a room\n";
            print "id: ", $room->{id}, "\n";
            print "msg: ", $self->unencrypt($room), "\n\n";
            $id_sum += $room->{id};
        }
    }
...
sub unencrypt {
    ...
    my $msg = '';
    for my $l (split '', $room->{message}) {
        if ($l eq '-') {
            $msg .= ' ';
        } else {
            my $decrypt_index = ($self->letters->{$l} + $room->{id}) % 26;

            $msg .= ($self->a2z)[$decrypt_index];
        }
    }
...
sub is_real_room {
    ...
    my $last_letter = '';
    my $highest_freq_index = 0;
    for my $csl (split '', $room->{checksum}) {
        my $freq_patt = $room->{by_highest_frequency}->[$highest_freq_index]->{pattern};

        return 0 if !$last_letter && $csl !~ /^($freq_patt)$/;

        if ($last_letter) {
            if ($csl !~ /^($freq_patt)$/) {
                $freq_patt = $room->{by_highest_frequency}->[++$highest_freq_index]->{pattern};

                return 0 if $csl !~ /^($freq_patt)$/;
            } else { # letter is same frequency as last letter, make sure it is ordered alphabetically
                return 0 if $self->letters->{$csl} < $self->letters->{$last_letter};
            }
        }
...
sub to_room {
    ...
    $hash->{frequency} = $self->letter_freq($hash->{letters});

    my $by_highest_freq = [];
    for my $f (reverse sort {$hash->{frequency}->{$a}->{count} <=> $hash->{frequency}->{$b}->{count}} keys %{$hash->{frequency}}) {
        if (!scalar(@$by_highest_freq) || $hash->{frequency}->{$f}->{count} != $by_highest_freq->[-1]->{count}) {
            push @$by_highest_freq, { count => $hash->{frequency}->{$f}->{count}, pattern => $f };
        }

        $by_highest_freq->[-1]->{pattern} .= "|$f";
    }

    $hash->{by_highest_frequency} = $by_highest_freq;
