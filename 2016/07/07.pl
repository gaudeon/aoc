...
sub run {
    ...
    my $ips_supporting_TLS = 0;

    my $ips_supporting_SSL = 0;

    for my $data (@data) {
        if ($data =~ /(\w)(?!\1)(\w)\2\1/ && $data !~ /\[[^\[\]]*(\w)(?!\1)(\w)\2\1[^\[\]]*\]/) {
            $ips_supporting_TLS++;
        }

        my @outside_matches = $data =~ m/(?:^|\])[^\[\]]*((\w)(?!\2)\w\2)[^\[\]]*(?:$|\[)/;
        my @inside_matches  = $data =~ m/\[[^\[\]]*((\w)(?!\2)\w\2)[^\[\]]*\]/;
        ...
        for my $om (@outside_matches) {
            next unless length $om == 3;
            $outside_hash{$om}++;
        }

        for my $im (@inside_matches) {
            next unless length $im == 3;
            $inside_hash{$im}++;
        }

        debug $data, \%inside_hash, \%outside_hash;
        print "\n\n";

        my $has_both = 0;
        $has_both +=  $inside_hash{$_} && $outside_hash{$_} ? 1 : 0 for keys %inside_hash;
    ...
    print $ips_supporting_TLS, "\n";
    print $ips_supporting_SSL, "\n";
}
