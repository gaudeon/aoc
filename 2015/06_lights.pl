...
use constant TURN_ON  => 1000;
use constant TURN_OFF => 2000;
use constant TOGGLE   => 3000;
use constant GRID_WIDTH  => 999;
use constant GRID_HEIGHT => 999;
...
while(<DATA>) {
    ...
    for(my $y = $data->{orig}{y}; $y <= $data->{dest}{y}; $y++) {
        for(my $x = $data->{orig}{x}; $x <= $data->{dest}{x}; $x++) {
            my $index = $x + ($y * GRID_WIDTH);

            # Part 1
            #$grid[$index] = $data->{state} == TURN_ON  ? 1 :
            #                $data->{state} == TURN_OFF ? 0 :
            #                ! $grid[$index];

            # Part 2
            $grid[$index] -= 1 if $data->{state} == TURN_OFF;
            $grid[$index] += 1 if $data->{state} == TURN_ON;
            $grid[$index] += 2 if $data->{state} == TOGGLE;
            $grid[$index] = 0 if $grid[$index] < 0;
        }
    }
}

# Part 1
#my $lights_on  = scalar grep { $_ == 1 } @grid;
#my $lights_off = scalar grep { $_ != 1 } @grid;

#print "SIZE: ", GRID_WIDTH * GRID_HEIGHT, "\n";
#print "Lights On: ", $lights_on, "\n";
#print "Lights Off: ", $lights_off, "\n";

# Part 2
my $sum_brightness = 0;
$sum_brightness += $_ for @grid;
print "Intensity: ", $sum_brightness, "\n";
