...
my $code_char_count = 0;
my $mem_char_count = 0;

my $slash_quote   = qr/\\\"/;
my $slash_slash   = qr/\\\\/;
my $slash_hex     = qr/\\x/;
my $slash_hex_num = qr/\\x([0-9a-f][0-9a-f])/;
...
while(<DATA>) {
    ...
    while (scalar @chars) {
        ...
        if($chr eq '\\') {
            $chr .= shift @chars if scalar @chars;
            SWITCH: {
                $chr =~ /$slash_quote/ && do {
                    $chr = '"';
                    last SWITCH;
    ...
    $mem_char_count += length($line);

    print $line, " - ", length($line), "\n\n";
}

print "CODE: ", $code_char_count, "\n";
print "MEM: ", $mem_char_count, "\n";
print "DIFF: ", $code_char_count - $mem_char_count, "\n";
